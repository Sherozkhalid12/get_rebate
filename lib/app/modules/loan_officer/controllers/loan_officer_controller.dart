...
class LoanOfficerController extends GetxController {
  ...
  static const String _zipCodesCacheTimestampPrefix =
      'loan_officer_zip_codes_timestamp_';
  ...
  Future<void> claimZipCode(ZipCodeModel zipCode) async {
    try {
      ...
      final requestBody = {
        ...
        'price': (zipCode.price ?? zipCode.calculatedPrice).toStringAsFixed(
          0,
        ), // Convert to string as per API
        'state': zipCode.state,
      ...
      NetworkErrorHandler.handleError(
        e,
        defaultMessage:
            'Unable to claim ZIP code. Please check your internet connection and try again.',
      );
    } finally {
      _isLoading.value = false;
    }
