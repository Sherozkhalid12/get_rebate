...
/** Sample addendum / disclosure wording (matches the mobile app rebate checklist). */

export const BUYER_REPRESENTATION_ADDENDUM = `The rebate is typically based only on the total Buyer Agent Commission (BAC) negotiated/ paid by the seller or builder. BAC is now negotiated on a property-by-property basis. Real Estate commission is 100% negotiable.
...
export const PURCHASE_OFFER_DISCLOSURE = `Real Estate Commission Rebate and Commission Split Disclosure:
...
export const LISTING_AGREEMENT_ADDENDUM = `Listing/Selling Commission Rebate Addendum/Amendment
...
<style>
  body { font-family: Georgia, "Times New Roman", serif; padding: 32px; color: #111; line-height: 1.5; }
...
export function SampleFormActions({ title, body, accent = 'blue' }) {
  ...
  return (
    <div className="sample-form-actions">
      <button type="button" className={`btn outline small sample-form-link sample-form-link--${accent}`} onClick={() => setOpen(true)}>
        View sample form
      </button>
      {open ? (
        <div className="modal-overlay" onClick={() => setOpen(false)} role="presentation">
          <div className="modal-dialog glass-card sample-form-dialog" onClick={(e) => e.stopPropagation()} role="dialog" aria-labelledby="sample-form-title">
            <div className="modal-header">
              <h3 id="sample-form-title">{title}</h3>
              <button type="button" className="btn ghost icon" onClick={() => setOpen(false)} aria-label="Close">×</button>
            </div>
            <div className="modal-body">
              <pre className="sample-form-body">{body}</pre>
            </div>
            <div className="sample-form-dialog-footer">
              <button type="button" className="btn ghost" onClick={() => setOpen(false)}>Close</button>
              <button type="button" className="btn outline" onClick={() => printDocument(title, body)}>Print</button>
...
export function BuyingChecklistItem({ index, text }) {
  const step = index + 1;
  return (
    <li className="checklist-item">
      <span className="checklist-num">{step}</span>
      <div className="checklist-item-body">
        <span className="checklist-text">{text}</span>
        {step === 2 ? (
          <SampleFormActions
            title="Buyer Representation Agreement Addendum (Step #2)"
            body={BUYER_REPRESENTATION_ADDENDUM}
          />
        ) : null}
        {step === 9 ? (
          <SampleFormActions
            title="Purchase Offer Rebate Disclosure (Step #9)"
            body={PURCHASE_OFFER_DISCLOSURE}
          />
        ) : null}
      </div>
    </li>
...
export function SellingChecklistItem({ index, text }) {
  const step = index + 1;
  return (
    <li className="checklist-item">
      <span className="checklist-num">{step}</span>
      <div className="checklist-item-body">
        ...
        {step === 2 ? (
          <SampleFormActions
            title="Listing Agreement Rebate Disclosure (Step #2)"
            body={LISTING_AGREEMENT_ADDENDUM}
            accent="green"
          />
        ) : null}
      </div>
